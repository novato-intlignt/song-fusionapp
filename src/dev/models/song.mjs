...
export class SongModel {
  ...
  static async check (id) {
    ...
    const checkSong = await connection.execute('SELECT full_title FROM songs WHERE id_api = ?', [idApi])

    const valueStatus = (checkSong[0].length > 0) ? 'exist' : 'none'
    const fullTitle = (checkSong[0].length > 0) ? checkSong[0][0].full_title : 'nothing'
    return { status: valueStatus, title: fullTitle }
  }
